...
describe("buildBot handler loader", () => {
  ...
  it("unknown input is handled by the question-answer handler", async () => {
    const suite = await runSpecs(() => buildBot("test-token"), [
      parseBotSpec({
        name: "unknown text gets a question-answer response",
        steps: [
          { send: { text: "qwerty" },
            expect: [{ method: "sendMessage", payload: { text: "Interesting question! Could you provide more details so I can help better?" } }] },
        ],
      }),
    ]);
    expect(suite.failed).toBe(0);
